...
<body class="bg-gradient-primary">
		...
		$(function() {
			...
			$('button').on("click",
					function(e)
					{
						...
						if (operation === 'remove')
							{
								formObj.attr("action", "/member/premove");
							}
						else if (operation === 'list')
							{
								formObj.find("#p_id").remove();
								formObj.attr("action", "/notice/mainhome")
							}
							formObj.submit();
						});
			});
	...
	<div class="container">
		...
		<h1 class="sign__title">회원정보 보기</h1>
		...
		<div class="sign-tab__content sign-tab__content-active user-info">
				...
				<div class="input-box-wrap">
					<div class="input-box id">
						<input type="text" id="userid" name="p_id" class="has-label"
							value='<c:out value="${member.p_id }" />' maxlength="15"
			...
			<section class="sign-section passwd">
				<ul class="input-box-wrap">
					<li class="input-box"><input type="password" id="passwd"
						name="p_password" class="has-label" value="" required
						autocomplete="new-password" readonly="readonly"> <label
						for="passwd">비밀번호 <span>8~16자 영문,숫자,특수문자 조합</span>
					</label></li>
					<li class="input-box"><input type="password" id="passwd2"
						name="p_password_con" class="has-label" value="" maxlength="16"
						required autocomplete="new-password"> <label for="passwd2"
						readonly="readonly">비밀번호 재입력</label></li>
				</ul>
				<div class="valid-text">
			...
			<section class="sign-section name">
				<div class="input-box-wrap">
					<div class="input-box">
						<input type="text" id="usernm" name="p_name" class="has-label"
							value="<c:out value="${member.p_name }" />" maxlength="20"
							readonly="readonly"> <label for="usernm">이름</label>
					</div>
				</div>
				<div class="valid-text">
			...
			<section class="sign-section birth">

				<div class="input-box-wrap">
					<div class="input-box fixed">
						<label for="userid" class="fixed-label">생년월일</label>
						<p class="fixed-data"></p>
					</div>
				</div>

				<div class="select-gender gender">
					<input type="radio" id="male" name="p_sex" value="남자"
						<c:if test="${member.p_sex eq '남자'}"> <c:out value="checked"/> </c:if>
						onclick="return(false);"><label for="male">남</label> <input
						type="radio" id="female" name="p_sex" value="여자"
						<c:if test="${member.p_sex eq '여자'}"> <c:out value="checked"/> </c:if>
			...
			<section class="sign-section email">
				<ul class="input-box-wrap">
					<li class="input-box hide-last-child"><input type="text"
						id="email" class="has-label" name="p_email"
						value="<c:out value="${member.p_email }" />" maxlength="50"
						autocomplete="off" readonly="readonly"> <label
						class="fixed-label" for="birthdate">이메일</label></li>
				</ul>
				<!-- 이메일 자동완성 기능 -->
			...
			<section class="sign-section mobile">
				<ul class="input-box-wrap">
					<li class="input-box hide-last-child"><input type="tel"
						id="htel" class="has-label" name="p_phone"
						value="<c:out value="${member.p_phone }" />" maxlength="11"
						readonly="readonly"> <label for="htel">휴대폰 (- 제외
							번호 입력)</label></li>
				</ul>
			...
			<section class="sign-section">
				<ul class="input-box-wrap">
					<li class="input-box"><input type="text" id="addr1"
						name="p_address" class="has-label"
						value="<c:out value="${member.p_address }" />" readonly="readonly"
				...
				<div class="sign-section__content">
					<div class="radio-group">
						<span class="input"> <input type="radio"
							...
							비로그인 시에도 <strong>계정 활성 상태 유지</strong>
						</span>
					</div>
				</div>
			</section>
			...
			<form id='operform' action="/member/pmodify" method="get">
				<input type="hidden" id='p_id' name='p_id'
					value='<c:out value="${member.p_id}"/>'>
			</form>
		</div>
	</div>
</body>
